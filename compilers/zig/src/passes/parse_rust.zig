...
const Parser = struct {
    ...
    fn parseFunction(self: *Parser, is_public: bool) MethodNode {
        ...
        var method_loc = self.currentSourceLoc();
        if (self.current.kind == .ident) {
            const name_tok = self.bump();
            raw_name = name_tok.text;
            method_loc = self.tokenSourceLoc(name_tok);
        } else {
        ...
        if (self.expect(.lparen) == null) {
            return .{ .name = name, .is_public = is_public, .params = &.{}, .body = &.{}, .source_loc = method_loc };
        }
        ...
        return .{ .name = name, .is_public = is_public, .params = params.items, .body = body.items, .source_loc = method_loc };
    }
    ...
    fn parseAssertMacro(self: *Parser) ?Statement {
        ...
        _ = self.expect(.rparen);
        _ = self.match(.semicolon);
        return .{ .assert_stmt = .{ .condition = expr, .source_loc = loc } };
    }
    ...
    fn parseAssertEqMacro(self: *Parser) ?Statement {
        ...
        return .{ .assert_stmt = .{ .condition = .{ .binary_op = bop }, .source_loc = loc } };
    }
    ...
    fn parseLetDecl(self: *Parser) ?Statement {
        ...
        if (mutable) {
            ...
            return .{ .const_decl = .{ .name = var_name, .type_info = ti, .value = val, .source_loc = loc } };
        }
    }
    ...
    fn parseIfStmt(self: *Parser) ?Statement {
        ...
        return .{ .if_stmt = .{ .condition = cond, .then_body = then_stmts.items, .else_body = else_body, .source_loc = loc } };
    }
