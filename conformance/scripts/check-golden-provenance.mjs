...
//       modifies that fixture's `conformance/witnesses/<fixture>.json` AND pins
...
function checkProvenance({ changedFiles, root }) {
  ...
  for (const g of goldenChanges) {
    ...
    if (witness && changedSet.has(witness)) {
      ...
      if (existsSync(witnessAbs) && existsSync(goldenAbs)) {
        ...
        if (pin === actual) {
          justified.push({
            path: g,
            via: 'cross-check',
            detail: `content-pinned in ${witness} goldenHashes["${key}"]`,
          });
          continue;
        }
      }
      // Witness co-changed but did not pin the new golden content — not justified
      // via path A. Fall through to path B (allowlist).
    }
    ...
    unjustified.push({
      path: g,
      reason: witness
        ? `witness ${witness} co-change must pin the new golden in ` +
